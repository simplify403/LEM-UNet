...
import torch
from nnunet_LEM.training.loss_functions.TopK_loss import TopKLoss
from nnunet_LEM.training.loss_functions.crossentropy import RobustCrossEntropyLoss
from nnunet_LEM.utilities.nd_softmax import softmax_helper
from nnunet_LEM.utilities.tensor_utilities import sum_tensor
...
class DC_and_CE_loss(nn.Module):
    ...
    def forward(self, net_output, target):
        ...
        if net_output.shape[1] == 4:
            split_tensors = torch.split(net_output, 2, dim=1)
            x = split_tensors[0]
            dc_loss = self.dc(x, target, loss_mask=mask) if self.weight_dice != 0 else 0
            if self.log_dice:
                dc_loss = -torch.log(-dc_loss)
            ce_loss = self.ce(x, target[:, 0].long()) if self.weight_ce != 0 else 0
            if self.ignore_label is not None:
                ce_loss *= mask[:, 0]
                ce_loss = ce_loss.sum() / mask.sum()
            x_edge = split_tensors[1]
            structure = ndimage.generate_binary_structure(3, 3)
            target = torch.squeeze(target)#x,x,x
            target = target.detach().cpu().numpy()
            erosion = ndimage.binary_erosion(target, structure=structure)
            erosion = numpy.where(erosion >= 1, 1.0, 0.0)
            boundaries = target - erosion
            boundaries = torch.from_numpy(boundaries).cuda()
            boundaries_1 = boundaries.unsqueeze(0)
            boundaries_1_1 = boundaries_1.unsqueeze(0)#1,1,x,x,x
            edge_loss = self.dc(x_edge, boundaries_1_1, loss_mask=mask) if self.weight_dice != 0 else 0
            if self.log_dice:
        ...
        if self.aggregate == "sum":
            if edge_loss == 0:
                ...
                result = 0.7 * (self.weight_ce * ce_loss + self.weight_dice * dc_loss) + 0.3 * edge_loss
            # print(result)
        else:
